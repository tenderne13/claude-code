...
当用户输入与历史记忆可能相关时，系统会通过 `startRelevantMemoryPrefetch()` 触发相关记忆检索。

#### 触发者与调用位置

**入口函数**：`startRelevantMemoryPrefetch()`（位于 `src/utils/attachments.ts:2362`）

**触发时机**：主查询循环在每次用户发送新消息、开始一轮新对话时通过 `using` 绑定调用。

#### 触发条件判断逻辑

该函数有四层过滤条件，全部通过才会执行召回：
...
findRelevantMemories()                      // findRelevantMemories.ts:39
  ...
  ├─ selectRelevantMemories()               // 调用 Sonnet 模型选择
