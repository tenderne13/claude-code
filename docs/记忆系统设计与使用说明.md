...
```mermaid
graph TD
    A["用户输入"] --> B["QueryEngine / Context Builder"]
    B --> C[getUserContext]
    C --> D[getMemoryFiles]
    D --> E1["Managed/User/Project/Local CLAUDE.md"]
    D --> E2["AutoMem MEMORY.md"]
    D --> E3["TeamMem MEMORY.md"]
    C --> F["拼接到对话上下文"]

    A --> G["运行时相关记忆召回"]
    G --> H[findRelevantMemories]
    H --> I["scanMemoryFiles + relevance selection"]
    I --> J["relevant_memories attachment"]
    J --> F

    A --> K["模型执行"]
    K --> L{是否主动写记忆}
    L -->|是| M["直接写入 memory 目录"]
    L -->|否| N[stopHooks]
    N --> O["extractMemories 后台子代理"]
    O --> P["写入 auto memory/topic 文件 + MEMORY.md"]

    Q["/memory 命令"] --> R[MemoryFileSelector]
    R --> E1
    R --> E2
    R --> E3

    S[AgentTool] --> T[agentMemory]
    T --> U["agent-memory 目录"]
```
...
当用户输入与历史记忆可能相关时，系统会调用 `findRelevantMemories()`：

1. 先扫描 memory 目录下的 `.md` topic 文件头部信息
2. 用 `description`、文件名等构造 manifest
3. 让模型从中挑最多 5 个最相关文件
4. 读取这些文件的前若干行或受字节限制的内容
5. 以 `relevant_memories` attachment 形式注入当前回合

这一步是真正意义上的“记忆检索”。

如果用户显式 @ 某个 agent，则只在该 agent 的记忆目录中检索；否则默认查 auto memory 目录。
