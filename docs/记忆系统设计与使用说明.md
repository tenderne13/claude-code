...
同时，本文新增了一个演示视角：如何结合 `memory_design/react_agent_demo/` 中的 Python Demo，把 Claude Code 里的真实记忆机制拆成几个可以现场跑通、可观察的最小能力。
...
如果你要给同事演示，建议把本文分成两层来讲：

1. 先讲 Claude Code 真实实现：说明正式工程里有哪些记忆分层、何时加载、何时回写。
2. 再讲 `react_agent_demo`：说明 Demo 没有 1:1 复刻全部能力，但已经能稳定演示最核心的四条链路：
   - `MEMORY.md` 索引加载
   - `search_memories` / `read_memory` 按需召回
   - `upsert_memory` 手动写入长期记忆
   - 回合结束后的 Auto Memory / `extractMemories` 自动回写

这样讲的好处是：

- 同事先理解真实架构
- 再看一个更小、更容易跑通的实现
- 现场可以直接打开对应代码文件解释，而不是只停留在概念层
...
### 3.1 `react_agent_demo` 中的对应模块

为了方便你演示，下面给出一份“真实能力 -> Demo 对应实现”的映射表。

| Claude Code 能力 | `react_agent_demo` 对应文件 | 演示点 |
|---|---|---|
| `MEMORY.md` 作为长期记忆索引 | `memory/memory_store.py`、`memory/index_manager.py` | 展示索引初始化、索引登记、按索引加载 |
| 相关记忆召回 | `memory/memory_search.py`、`context/context_builder.py` | 展示 query 如何先召回，再拼进上下文 |
| 记忆工具调用 | `tools/definitions/*.py` | 展示 `list/search/read/upsert/delete` 五类操作 |
| ReAct 主循环 | `agent/agent_loop.py` | 展示 Thought / Action / Observation / Final Answer |
| 主动写记忆 | `tools/definitions/upsert_memory.py` | 展示模型或用户显式写入长期记忆 |
| `extractMemories` 回合后抽取 | `memory/auto_memory.py`、`agent/agent_loop.py` | 展示回合结束后的自动抽取与写回 |
| Mock / HTTP 两种模型模式 | `llm/client.py` | 现场演示建议优先用 `mock`，保证稳定 |
...
在 `react_agent_demo` 中，对应的是：

- `tools/definitions/upsert_memory.py`
- `memory/memory_store.py`
- `memory/index_manager.py`

也就是说，Demo 里如果模型决定调用 `upsert_memory`，会直接：
...
在 `react_agent_demo` 中，这条链路已经补成了一个简化版实现，对应文件：
...
1. ReAct 循环结束后，`AgentLoop` 调用 `AutoMemoryManager.run_post_turn_extraction()`
2. 如果本轮已经显式调用过 `upsert_memory`，则跳过自动抽取，避免重复写入
3. 否则由 `extract_memories()` 从用户输入中抽取稳定偏好、反馈约束、项目事实
4. 最后通过 `MemoryStore.upsert_memory()` 写回 `demo_memory_store/` 并同步更新 `MEMORY.md`

这不是正式工程里完整的子代理版 `extractMemories`，但非常适合现场演示“stop hook 之后自动回写 durable memory”这个核心思想。
...
在 `react_agent_demo` 中，这条链路对应的是：

- `context/context_builder.py`：每轮开始时用当前问题做召回
- `memory/memory_search.py`：按关键词重叠进行打分
- `tools/definitions/search_memories.py`：暴露给 Agent 的搜索工具
- `tools/definitions/read_memory.py`：读取最相关记忆正文
- `agent/react_agent.py`：注册 `search_memories` 和 `read_memory`

建议演示时重点强调两点：

1. `ContextBuilder.build()` 会先把 `MEMORY.md` 和召回结果塞进 system prompt，模拟“预取式注入”
2. Agent 在推理过程中还可以继续调用 `search_memories` / `read_memory`，模拟“按需精查”
...
## 11.0 用 `react_agent_demo` 演示的推荐顺序

下面这组命令适合现场从“查询 -> 显式写入 -> 自动抽取”逐步演示：
...
# 3. 演示显式写记忆（模型调用 upsert_memory）
python3 main.py chat "记住：以后请优先给我 Java 示例，回答里不要用 emoji" --llm-mode mock --verbose

# 4. 演示 Auto Memory / extractMemories
python3 main.py chat "我不喜欢韭菜，以后推荐菜时避开" --llm-mode mock --verbose

# 5. 演示关闭 Auto Memory 后的对比
python3 main.py chat "我不喜欢香菜，以后推荐菜时避开" --llm-mode mock --verbose --disable-auto-memory

# 6. 查看最终 MEMORY.md 和 topic 文件
...
- `agent/agent_loop.py`
- `memory/auto_memory.py`
- `tools/definitions/search_memories.py`
- `tools/definitions/upsert_memory.py`
...
### 15.1 `react_agent_demo` 关键源码索引

| 主题 | 文件 |
|---|---|
| CLI 入口 | `memory_design/react_agent_demo/main.py` |
| ReAct 主循环 | `memory_design/react_agent_demo/agent/agent_loop.py` |
| 工具注册 | `memory_design/react_agent_demo/agent/react_agent.py` |
| 上下文构建 | `memory_design/react_agent_demo/context/context_builder.py` |
| 记忆存储主入口 | `memory_design/react_agent_demo/memory/memory_store.py` |
| `MEMORY.md` 维护 | `memory_design/react_agent_demo/memory/index_manager.py` |
| 记忆搜索 | `memory_design/react_agent_demo/memory/memory_search.py` |
| Auto Memory / extractMemories 演示版 | `memory_design/react_agent_demo/memory/auto_memory.py` |
| 搜索工具 | `memory_design/react_agent_demo/tools/definitions/search_memories.py` |
| 写入工具 | `memory_design/react_agent_demo/tools/definitions/upsert_memory.py` |

## 16. 一句话总结

当前项目的记忆系统，本质上是“`CLAUDE.md` 指令系统 + auto memory 长期事实库 + 回合后后台抽取 + 运行时按需召回 + agent 独立记忆”的组合架构；而 `react_agent_demo` 现在已经可以稳定演示其中最核心的三件事：`MEMORY.md` 索引加载、`search/read/upsert` 工具化记忆操作，以及回合结束后的 Auto Memory / `extractMemories` 自动回写。
